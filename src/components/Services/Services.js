    ...
    const serviceItems = ServiceData.map(item => 
        <Card key={item.id} sx={Styles.cardStyle} >
            ...
            <CardContent sx={Styles.cardContent1} id="card1">
                {item.icon}
                <Typography textAlign={i18n.language === 'en'? 'left':'right'} variant="h2" fontWeight='500' lineHeight='50px' maxWidth='277px'>{t(item.name)}</Typography>
            </CardContent>
            <CardContent sx={Styles.cardContent2} id="card2">
                <Typography textAlign={i18n.language === 'en'? 'left':'right'} fontSize='28px' lineHeight='33px'>{t(item.text)}</Typography>
            </CardContent>
